...
const Dashboard = () => {
  ...
  const getUserPrivileges = async () => {
    const response = await axiosInstance.get("/user-privileges-matrix");
    if (response?.status === 200) {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
        ...
        {activeMenu === "User Management" && (
          <div className="w-4/5">
            <div className="flex w-full justify-between">
              ...
              <div>
                <button
                  onClick={() => setIsNewUSerDrawer(true)}
                  className=" cursor-pointer bg-[var(--accent)] text-white px-4 py-2 rounded-lg hover:bg-blue-700"
                >
                  Create New User
                </button>
              </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-blue-100 border-b border-gray-300">
                    <th className="text-left p-2  border-gray-300">
                      First Name
                    </th>
                    <th className="text-left p-2 border-l border-gray-300">
                      Last Name
                    </th>
                    <th className="text-left p-2 border-l border-gray-300">
                      Email
                      ...
                      <td className="p-2 border-b border-l border-gray-300 text-gray-500">
                        <div className="flex justify-center">
                          {editingId === index ? (
                            <>
                              <i
                                className="fa-solid fa-check mr-[20px] text-green-500 hover:text-green-600 cursor-pointer"
                                onClick={() => updateUser(index)}
                              ></i>
                              <i
                                className="fa-solid fa-xmark text-red-500 hover:text-red-600 cursor-pointer"
                            ...
                            <>
                              <i
                                className="fa-solid fa-pen-to-square mr-[20px] text-gray-300 hover:text-[var(--accent)] cursor-pointer"
                                onClick={() => toggleEdit(index)}
                              ></i>
                              <i
                                onClick={() => removeUser(index)}
                                className="fa-solid fa-trash text-gray-300 hover:text-red-400 cursor-pointer"
                              ></i>
                            </>
                          )}
                        </div>
        ...
        {activeMenu === "Privilege Matrix" && (
          <div className="w-4/5">
            <div className="flex w-full justify-between mb-2">
              ...
              <div>
                <button
                  onClick={() => setIsNewResourceDrawer(true)}
                  className="bg-[var(--accent)] cursor-pointer text-white px-4 py-2 rounded-lg hover:bg-blue-700"
                >
                  Create New Resource
                </button>
              </div>
              ...
              <div className="w-1/4 bg-white p-4 rounded-lg shadow-md">
                ...
                <ul>
                  {userRoles.map((role, index) => (
                    <li
                      key={index}
                      className={`p-2 border-b border-gray-300 last:border-b-0  cursor-pointer ${
                ...
                <table className="w-full ">
                  <thead>
                    <tr className="bg-blue-100 border-b border-gray-300">
                      ...
                      <th className="text-center p-2 border-l border-gray-300">
                        View
                      </th>
                      <th className="text-center p-2 border-l border-gray-300">
                        Create
                      ...
                      <tr key={moduleIndex}>
                        ...
                        {["view", "create", "edit", "delete"].map(
                          (permission, permIndex) => (
                            <td
                              className="p-2 border-b border-gray-300 border-l border-gray-300 text-center"
                              key={permIndex}
                            >
                              <input
                                type="checkbox"
      ...
      {isDrawerOpen && (
        <div
          className="fixed h-full inset-0 bg-black opacity-20 z-40"
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[400px] bg-white shadow-lg z-50 transform ${
          isNewUserDrawer ? "translate-x-0" : "translate-x-full"
        } transition-transform duration-300`}
      >
        <div className="p-6 h-full flex flex-col justify-between">
          <div className="flex justify-between items-center mb-6">
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[400px] bg-white shadow-lg z-50 transform ${
          isNewResourceDrawer ? "translate-x-0" : "translate-x-full"
        } transition-transform duration-300`}
      >
        <div className="p-6 h-full flex flex-col justify-between">
          <div className="flex justify-between items-center mb-6">
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  id="Resource"
                  onChange={(e) =>
                    setNewResource({ ...newResource, code: e.target.value })
                  }
